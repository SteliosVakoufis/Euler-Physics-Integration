...
const drag = 0.9;
const maxPspeed = 8;
const onClickAttractionForce = 1;
...
function attractParticles(pos, p) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        p.children[i].acceleration.x = (onClickAttractionForce * force.x) + (p.children[i].acceleration.x * 0.98);
        p.children[i].acceleration.y = (onClickAttractionForce * force.y) + (p.children[i].acceleration.y * 0.98);
    }
}

function attractParticlesDef(pos, p) {
    let len = p.children.length;
    for (let i = 0; i < len; i++) {
        let force = { x: pos.x - p.children[i].x, y: pos.y - p.children[i].y };
        force = normalizeVector(force);
